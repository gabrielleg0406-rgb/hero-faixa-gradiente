...
const DirLight: FC<{ position: [number, number, number]; color: string }> = ({ position, color }) => {
  ...
  return <directionalLight ref={dir} castShadow position={position} intensity={6} color={color} />
}
...
export { Beams, CanvasWrapper }

export default function EtherealBeamsBackground() {
  return (
    <div className="absolute inset-0 w-full h-full">
      <CanvasWrapper>
        <Beams lightColor="#22c55e" speed={1.5} beamWidth={1.5} beamNumber={14} scale={0.25} rotation={0} />
      </CanvasWrapper>
    </div>
  )
}
